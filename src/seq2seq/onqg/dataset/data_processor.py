...
def preprocess_dialogue_batch(batch, enc_rnn=False, dec_rnn=False,
                              copy=False, attn_mask=False, device=None):
    ...
    if copy:
        f_copy_gold, f_copy_switch = batch['copy']['forward'][1], batch['copy']['forward'][0]
        f_copy_gold, _ = preprocess_input(f_copy_gold, device=device)
        f_copy_switch, _ = preprocess_input(f_copy_switch, device=device)
        f_copy_gold, f_copy_switch = f_copy_gold[:, 1:], f_copy_switch[:, 1:]
    
    b_copy_gold, b_copy_switch = None, None
    if copy:
        b_copy_gold, b_copy_switch = batch['copy']['backward'][1], batch['copy']['backward'][0]
        b_copy_gold, _ = preprocess_input(b_copy_gold, device=device)
        b_copy_switch, _ = preprocess_input(b_copy_switch, device=device)
        b_copy_gold, b_copy_switch = b_copy_gold[:, 1:], b_copy_switch[:, 1:]
        
    return inputs, gold, (f_copy_gold, f_copy_switch), (b_copy_gold, b_copy_switch)
