...
class DialogueDataset(object):
    ...
    def __init__(self, datasets, batchSize, copy=False, 
        ...
        self.copy = copy
        self.forward_copy_switch = datasets['copy']['forward']['switch'] if copy else None
        self.forward_copy_tgt = datasets['copy']['forward']['tgt'] if copy else None
        self.backward_copy_switch = datasets['copy']['backward']['switch'] if copy else None
        self.backward_copy_tgt = datasets['copy']['backward']['tgt'] if copy else None
    ...
    def _update_data(self):
        ...
        if self.tgt:
            self.tgt = [self.tgt[idx] for idx in self.idxs]
        if self.copy:
            self.forward_copy_switch = [self.forward_copy_switch[idx] for idx in self.idxs]
            self.forward_copy_tgt = [self.forward_copy_tgt[idx] for idx in self.idxs]
            self.backward_copy_switch = [self.backward_copy_switch[idx] for idx in self.idxs]
            self.backward_copy_tgt = [self.backward_copy_tgt[idx] for idx in self.idxs]
    ...
    def __getitem__(self, index):
        ...
        copyForwardSwitchBatch, copyForwardTgtBatch = None, None
        copyBackwardSwitchBatch, copyBackwardTgtBatch = None, None
        if self.copy:
            copyForwardSwitchBatch = self._batchify(self.forward_copy_switch[index * self.batchSize: (index + 1) * self.batchSize])
            copyForwardTgtBatch = self._batchify(self.forward_copy_tgt[index * self.batchSize: (index + 1) * self.batchSize])
            copyBackwardSwitchBatch = self._batchify(self.backward_copy_switch[index * self.batchSize: (index + 1) * self.batchSize])
            copyBackwardTgtBatch = self._batchify(self.backward_copy_tgt[index * self.batchSize: (index + 1) * self.batchSize])
        ...
        if self.copy:
            rst['copy'] = {
                ...
                'backward': (wrap(copyBackwardSwitchBatch), wrap(copyBackwardTgtBatch))
            }
        if self.attn_mask:
            rst['attn_mask'] = [self.attn_mask_data[i] for i in idxBatch]
    ...
    def shuffle(self):
        ...
        def shuffle_group(start, end, NEW):
            ...
            if self.copy:
                fcpswt = [self.forward_copy_switch[start:end][i] for i in index]
                fcptgt = [self.forward_copy_tgt[start:end][i] for i in index]
                bcpswt = [self.backward_copy_switch[start:end][i] for i in index]
                bcptgt = [self.backward_copy_tgt[start:end][i] for i in index]
                NEW['FCOPYSWTs'] += fcpswt
                NEW['FCOPYTGTs'] += fcptgt 
                NEW['BCOPYSWTs'] += bcpswt
                NEW['BCOPYTGTs'] += bcptgt
        ...
        NEW = {'SRCs':[], 'TGTs':[], 'IDXs':[]}
        if self.copy:
            NEW['FCOPYSWTs'], NEW['FCOPYTGTs'] = [], []
            NEW['BCOPYSWTs'], NEW['BCOPYTGTs'] = [], []
        ...
        self.src, self.tgt, self.idxs = NEW['SRCs'], NEW['TGTs'], NEW['IDXs']
        if self.copy:
            self.forward_copy_switch, self.forward_copy_tgt = NEW['FCOPYSWTs'], NEW['FCOPYTGTs']
            self.backward_copy_switch, self.backward_copy_tgt = NEW['BCOPYSWTs'], NEW['BCOPYTGTs']
