...
def build_dialogue_model(opt, device, separate=-1, checkpoint=None):
    encoder = build_encoder(opt, separate=separate)
    forward_decoder = build_decoder(opt, encoder.word_emb.weight, device)
    backward_decoder = build_decoder(opt, encoder.word_emb.weight, device)
    
    model = Dialoguer(encoder, forward_decoder, backward_decoder)
    model.forward_generator = nn.Linear(opt.d_dec_model // opt.maxout_pool_size, opt.tgt_vocab_size, bias=False)
    model.backward_generator = nn.Linear(opt.d_dec_model // opt.maxout_pool_size, opt.src_vocab_size, bias=False)
